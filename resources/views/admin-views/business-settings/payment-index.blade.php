...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row digital_payment_methods mt-3 g-3">
            ...
            <div class="card">
                <div class="card-body p-30px">
                    @php($config=\App\CentralLogics\Helpers::get_business_settings('xendit'))
                    <form action="{{env('APP_MODE')!='demo'?route('admin.business-settings.third-party.payment-method-update',['xendit']):'javascript:'}}" method="post">
                        @csrf
                        @if(isset($config))
                        <h5 class="d-flex flex-wrap justify-content-between">
                            <strong>{{translate('messages.xendit')}}</strong>
                            <label class="switch--custom-label toggle-switch toggle-switch-sm d-inline-flex">
                                <span class="mr-2 switch--custom-label-text text-primary on text-uppercase">{{ translate('messages.on') }}</span>
                                <span class="mr-2 switch--custom-label-text off text-uppercase">{{ translate('messages.off') }}</span>
                                <input type="checkbox" class="toggle-switch-input" name="status" value="1" {{$config?($config['status']==1?'checked':''):''}}>
                        ...
                        <div class="payment--gateway-img">
                            <img class="xendit-logo" src="{{asset('/public/assets/admin/img/payment/xendit.png')}}" alt="public">
                        </div>
                        <div class="form-group mb-4">
                            <input class="form-control" type="text" placeholder="Public Key" name="public_key" value="{{env('APP_MODE')!='demo'?$config['public_key']:''}}">
                        </div>
                        <div class="form-group mb-4">
                            <input class="form-control" type="text" placeholder="Private Key" name="private_key" value="{{env('APP_MODE')!='demo'?$config['private_key']:''}}">
                        </div>
                        <div class="btn--container justify-content-end">
                            <button type="{{env('APP_MODE')!='demo'?'submit':'button'}}" onclick="{{env('APP_MODE')!='demo'?'':'call_demo()'}}" class="btn h--37px btn--primary">{{translate('messages.save')}}</button>
                        </div>
                        @else
                        <button type="submit" class="btn btn--primary mb-2">{{translate('messages.configure')}}</button>
