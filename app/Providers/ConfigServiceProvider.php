...
class ConfigServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        try {
            ...
            $data = BusinessSetting::where(['key' => 'xendit'])->first();
            $xendit = json_decode($data['value'], true);
            if ($xendit) {
                $config = array(
                    'xendit_key' => env('XENDIT_KEY', $xendit['xendit_key']),
                    'xendit_secret' => env('XENDIT_SECRET', $xendit['xendit_secret'])
                );
                Config::set('xendit', $config);
            }
